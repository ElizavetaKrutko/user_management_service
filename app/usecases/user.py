...
class UserUseCase:
    ...
    async def get_another_user_by_id(self, user_id_needed: uuid.UUID, user_data: schemas.UserInfo):
        db_user_needed = await self.db_repo.get_user_by_id(user_id=user_id_needed)
        logger.debug("we are in get_another_user_by_id")
        logger.debug(db_user_needed.role)
        logger.debug(user_data.role)
        ...
        if (user_data.role == Role.ADMIN or user_data.role == Role.MODERATOR) and db_user_needed.group_id == user_data.group_id:
